...
class _UserPaymentViewState extends State<UserPaymentView> with SingleTickerProviderStateMixin {
  ...
  Future<void> _finishTransaction(PaymentMethod method) async {
    ...
    if (method == PaymentMethod.transfer && _proofImage == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Wajib melampirkan foto bukti transfer!')),
      );
      return;
    }
    
    if (method == PaymentMethod.qris && _proofImage == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Wajib melampirkan foto bukti QRIS!')),
      );
      return;
    }
    ...
    if (_proofImage != null) {
      try {
        ...
        final base64String = base64Encode(await _proofImage!.readAsBytes());
        proofUrl = 'data:image/${_proofImage!.name.split('.').last};base64,$base64String';
      }
    }
    ...
    final tx = TransactionModel(
      ...
      cashierName: "Self Service - ${auth.currentUser?.name ?? 'User'}",
    ...
    final success = await trxProv.addTransaction(tx);
    ...
    if (!success) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Gagal menyimpan transaksi ke sistem (Database Error)!'),
                            ...
                            children: [
                              ...
                              SizedBox(
                                ...
                                child: FilledButton.icon(
                                  ...
                                  label: const Text('Konfirmasi Pembayaran Tunai', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                                ),
                              ),
                            ],
                          ),
